# Legacy comparison harness from an early prototype (predates the PPO
# pipeline). Not runnable as-is: it references a random_agent module and
# a NeuralNetworkAgent class that were renamed or removed before this
# script was archived. Kept for provenance only.
import compiler_gym
import json
import os
from datetime import datetime

from legacy.agents.greedy import GreedyAgent
from legacy.agents.random_agent import RandomAgent
from legacy.agents.nn_agent import NeuralNetworkAgent
from legacy.agents.dqn_agent import DQNAgent
...
def main():
    ...
    print(f"{'vs O3':<12} {'':>8} {(total_o3 - total_random) / total_o3 * 100:>+7.1f}% "
          f"{(total_o3 - total_greedy) / total_o3 * 100:>+7.1f}% "
          f"{(total_o3 - total_nn) / total_o3 * 100:>+7.1f}% "
          f"{(total_o3 - total_dqn) / total_o3 * 100:>+7.1f}%")
