# Legacy: traditional ML models trained on greedy-search trajectories,
# an approach superseded by the PPO agents in src/agents/ppo_*.py.
...
class MLAgent:
    ...
    def load_data(self, data_path="legacy/results/greedy_trajectories.json"):
    ...
    def train(self, data_path="legacy/results/greedy_trajectories.json"):
        """Full training pipeline."""
        print(f"-- ML Agent ({self.model_type}) on {self.benchmark}")
        ...
        print(f"\n   Result: {initial_ic} -> {final_ic} ({reduction:.1f}% reduction)")
...
def compare_models(data_path="legacy/results/greedy_trajectories.json"):
...
def run_on_all_benchmarks(model_type="random_forest", data_path="legacy/results/greedy_trajectories.json"):
...
if __name__ == "__main__":
    if not os.path.exists("legacy/results/greedy_trajectories.json"):
        print("Data not found. Run: python legacy/collect_and_save.py")
        exit(1)
