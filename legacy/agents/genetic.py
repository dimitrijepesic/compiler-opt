# Legacy genetic-algorithm agent, superseded by the PPO agents in
# src/agents/ppo_*.py. Kept for provenance only.
from src.agents.base_agent import OptimizerAgent
import random
import time
import copy
...
class GeneticAgent(OptimizerAgent):
    ...
    def train(self):
        print(f"--- running genetic agent on {self.env.benchmark} ---")
        ...
        print("-" * 50)
        print(f"genetic agent finished in {duration:.1f} seconds.")
