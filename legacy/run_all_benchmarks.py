# Legacy greedy-search sweep over the cBench-v1 suite, from before the
# controlled study existed. Superseded by scripts/run_full_baselines.py
# (real greedy search over the full action space) and
# scripts/benchmark_battery.py (the multi-suite battery). Kept for
# provenance only.
import compiler_gym
import json
...
def run_greedy(env, max_steps=45):
    """Greedy search: pick the best-reward action at each step."""
...
def main():
    ...
    filename = f"legacy/results/benchmarks_{timestamp}.json"
