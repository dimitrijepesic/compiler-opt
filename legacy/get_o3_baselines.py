# Legacy: -O3/-Oz approximated by a hand-picked pass sequence, the exact
# methodology the controlled study replaced with CompilerGym's real
# IrInstructionCountO3/Oz observations (see scripts/augment_baselines.py
# and the README's "What changed and why"). Kept for provenance only.
import compiler_gym
...
# Greedy results from an earlier run
GREEDY_RESULTS = {
    "adpcm": 218,
    "bitcount": 399,
    "blowfish": 1873,
...
def get_o3_instruction_count(benchmark):
    """Apply an approximate -O3 pass sequence and return the instruction count."""
    env = compiler_gym.make("llvm-ic-v0")
    env.reset(benchmark=benchmark)

    o3_passes = [
        "-mem2reg", "-instcombine", "-simplifycfg", "-early-cse", "-gvn",
        "-sccp", "-licm", "-indvars", "-loop-simplify", "-loop-rotate",
        "-loop-unroll", "-dce", "-adce", "-simplifycfg", "-instcombine",
    ]
...
def get_oz_instruction_count(benchmark):
    """Apply an approximate -Oz (size-focused) pass sequence."""
    env = compiler_gym.make("llvm-ic-v0")
    env.reset(benchmark=benchmark)
...
def main():
    ...
    with open("legacy/results/o3_comparison.json", "w") as f:
        json.dump(results, f, indent=2)

    print("\nResults saved to legacy/results/o3_comparison.json")
